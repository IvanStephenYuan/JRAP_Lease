...
public class ApiAccessLimitFilter extends OncePerRequestFilter {
    ...
    private static final String REDIS_CATALOG = BaseConstants.JRAP_CACHE + "access_limit:";
     ...
     * @throws JrapApiException  JrapApi异常
     * @throws IOException      流异常
     * @throws ServletException Servlet异常
     */
    private void checkAuth(String url, OAuth2Authentication oAuth2Authentication) throws JrapApiException, IOException, ServletException {
        // 分割url，serverUrl、interfaceUrl
        url = StringUtils.substringAfter(url, URL_HEAD);
        String[] temp = url.split("/");
        if (temp.length < 2) {
            throw new JrapApiException(JrapApiException.CODE_API_ACCESS_LIMIT, JrapApiException.MSG_ERROR_API_URL);
        }
        ...
        // server是否存在
        if (null == apiServer) {
            throw new JrapApiException(JrapApiException.CODE_API_ACCESS_LIMIT, JrapApiException.MSG_ERROR_API_NO_SERVER);
        }

        // server 是否启用
        if (!ApiConstants.ENABLE_FLAG_Y.equalsIgnoreCase(apiServer.getEnableFlag())) {
            throw new JrapApiException(JrapApiException.CODE_API_ACCESS_LIMIT, JrapApiException.MSG_ERROR_API_SERVER_DISABLE);
        }

        // 服务是否绑定client
        if (!oAuth2Authentication.getOAuth2Request().getScope().contains(apiServer.getCode())) {
            throw new JrapApiException(JrapApiException.CODE_API_ACCESS_LIMIT, JrapApiException.ERROR_NOT_FOUND);
        }
        ...
        if (null != apiInterface) {
            ...
            if (!ApiConstants.ENABLE_FLAG_Y.equalsIgnoreCase(apiInterface.getEnableFlag())) {
                throw new JrapApiException(JrapApiException.CODE_API_ACCESS_LIMIT, JrapApiException.MSG_ERROR_API_INTERFACE_DISABLE);
            }
        ...
        if (!checkAccessLimit(clientId, serverUrl, interfaceUrl, apiAccessLimit)) {
            throw new JrapApiException(JrapApiException.CODE_API_ACCESS_LIMIT, JrapApiException.MSG_ERROR_API_OVERSTEP);
        }
