...
public class DefaultProcessDiagramGenerator implements ProcessDiagramGenerator {
    ...
    protected void drawActivity(DefaultProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel,
        ...
        if (drawInstruction != null) {
            ...
            try {
                BeanUtils.copyProperties(nodeGraphicInfo, oriGraphicInfo);
                if (highLightedActivities.contains(flowNode.getId())) {
                    nodeGraphicInfo.setColor(new Color(173, 255, 47));
                } else if (highLightedActivities.contains(flowNode.getId() + IActivitiConstants.HISTORY_SUFFIX)) {
...
//                drawHighLightHis(processDiagramCanvas, bpmnModel.getGraphicInfo(flowNode.getId()), flowNode);
            }
        ...
        for (SequenceFlow sequenceFlow : flowNode.getOutgoingFlows()) {
            ...
            if (graphicInfoList != null && graphicInfoList.size() > 0) {
                ...
                if (labelGraphicInfo != null) {
                    ...
                    // 临时方案, 应该是一个 bug
                    List<GraphicInfo> list = bpmnModel.getFlowLocationGraphicInfo(sequenceFlow.getId());
                    GraphicInfo gi0 = list.get(0);
                    GraphicInfo gi1 = list.get(1);
                    List<GraphicInfo> first2 = list.subList(0, 2);
                    GraphicInfo center = getLineCenter(first2);
                    boolean hor = gi0.getY() == gi1.getY();
                    if (hor) {
                        // 两点水平
                        center.setY(center.getY() - 14);
                    }
    ...
    private static void drawHighLightHis(DefaultProcessDiagramCanvas processDiagramCanvas, GraphicInfo graphicInfo, FlowNode flowNode) {
        processDiagramCanvas.drawHighLightHis((int) graphicInfo.getX(), (int) graphicInfo.getY(),
                (int) graphicInfo.getWidth(), (int) graphicInfo.getHeight(), flowNode);
    ...
    protected static DefaultProcessDiagramCanvas initProcessDiagramCanvas(BpmnModel bpmnModel, String imageType,
        ...
        if ("svg".equalsIgnoreCase(imageType)) {
            return new SvgProcessDiagramCanvas((int) maxX + 10, (int) maxY + 10, (int) minX, (int) minY, imageType,
                    activityFontName, labelFontName, annotationFontName, customClassLoader);
        }
