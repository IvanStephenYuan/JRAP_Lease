...
activitiModeler
  ...
  .config(['$selectProvider', '$translateProvider', function ($selectProvider, $translateProvider) {
      ...
      angular.extend($selectProvider.defaults, {
        ...
        $translateProvider.preferredLanguage(window['currentLang']);

        // remember language
        //$translateProvider.useCookieStorage();
        $translateProvider.use(window['currentLang'])
            ...
            $rootScope.$on('$includeContentLoaded', function (event) {
	            if (!$rootScope.editorInitialized) {
	                ...
	                jQuery(window).resize(function () {
	                    ...
	                    canvas.height(totalAvailable - 15);
	                    jQuery('#paletteSection').height(totalAvailable);
	                    jQuery('#propertySection').height(totalAvailable);
	                    jQuery('#propertySection .selected-item-body').height(totalAvailable-73);
