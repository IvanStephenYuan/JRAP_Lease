...
ORYX.Core.StencilSet.StencilSet = Clazz.extend({
    ...
    construct: function(source, modelMetaData, editorId){
		...
		if(ORYX.CONFIG.BACKEND_SWITCH) {
			...
			//modify by hailin.xu@jingrui.com
			var url = KISBPM.URL.getStencilSet();
...
function init() {
    ...
    // Else fetch the model from server and display editor
		...
		var modelUrl = "./wfl/service/model/" + modelId + "/json";
...
new function(){
	...
	ORYX.Plugins.BPMN2_0 = {
        ...
        getLanes: function(shape, recursive){
				...
				var lanes = [];
				if(shape != null){
					lanes = shape.getChildNodes(recursive||false).findAll(function(node) { return (node.getStencil().id() === namespace + "Lane"); });
				}
