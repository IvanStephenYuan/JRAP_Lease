...
angular.module('activitiModeler')
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
        ...
        $scope.paletteWindowState = {'collapsed': false};
        $scope.canvasWindowState = {'collapsed': true};
        ...
        $scope.propertyWindowState.toggle = function () {
            $scope.propertyWindowState.collapsed = KISBPM.TOOLBAR.ACTIONS.collapsedProperty();
        };
        $scope.paletteWindowState.toggle = function () {
            $scope.paletteWindowState.collapsed = KISBPM.TOOLBAR.ACTIONS.collapsedPalette();
        };
        ...
        $scope.editorFactory.promise.then(function () {
            ...
            $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
                ...
                if (shapes && shapes.length > 0) {
                    ...
                    selectedItem.activityNodeTitle = stencil._jsonStencil.title;
                    ...
                    //add by hailin.xu@jingrui.com
                    var selectedItem = $scope.selectedItem.properties;
                    for(var item = 0;item<selectedItem.length;item++){
                         if($scope.selectedItem.properties[item]['key'] == "oryx-overrideid"){
                              var value = $scope.selectedItem.properties[item].value;
                              var uuid = "sid-"+$scope.uuid(16,10);
                              if(!value){
                                   $scope.selectedItem.properties[item].value = uuid;
                              }
                               $scope.updatePropertyInModel($scope.selectedItem.properties[item]);
                               break;
                          }
                   }
                    //add by hailin.xu@jingrui.com end
                   } else {
                    $scope.safeApply(function () {
                        $scope.selectedItem = {};
            		...
            		if (shapes && shapes.length == 1)
            		{
                        ...
                        var showSelectShapeDialog = function()
                        {
                            ...
                            $modal({
                                backdrop: false,
                                keyboard: true,
                                template: 'activiti-editor/popups/select-shape.html?version=' + Date.now()
                            });
                        };
        ...
        //add by hailin.xu@jingrui.com start
        //指派和表单通信
        //思路:将子作用域值存入父作用域中 其他子作用域直接访问父作用域访问
        $scope.assignment = [];
        $scope.setAssignment = function(property){ 
              var obj = {
                    id:"APPROVAL_CANDIDATE_RULE",
                    name:"",
                    expression:"",
                    readable:true,
                    type :"enum",
        	 ...
        	 var enumValues = [];
             if(property.value && property.value.assignment && property.value.assignment.length >0){
                  var assignment = property.value.assignment;
                  for(var i=0;i<assignment.length;i++){
                      var code = assignment[i].code;
                      var assigneeCode = assignment[i].assigneeCode;
                      var rules = assignment[i].rules;
                      var type = "";
                      if(code == "APPOINTED_EMPLOYEE"){
                          type =  "employeeCode"
                      }else if(code == "APPOINTED_POSITION"){
                          type = "positionCode";
                      ...
                      if(rules && rules.length){
                    	  for(var j=0;j<rules.length;j++){
                        	  if(j == rules.length-1){
                        		  a = a + '{"CODE":"'+rules[j].businessRuleCode+'"}';
                        	  }else{
                                  a = a + '{"CODE":"'+rules[j].businessRuleCode+'"},'; 
                        	  }
                          }
                      ...
                      var b='';
                      if(type && assigneeCode ){
                         b = '{"'+type+'":"'+assigneeCode+'"}';
                      }
                      enumValues.push({
                         id:code,
                         name:'{"parameter":['+b+'],"businessRules":['+a+']}',
                      })
                  }
                  
             }
             obj.enumValues = enumValues;
             $scope.syncFormProperties(obj,'APPROVAL_CANDIDATE_RULE');
             
        }
        
        
        $scope.setApproveStrategy = function(property){
        	var obj = {
        	 ...
        	 $scope.syncFormProperties(obj,'APPROVAL_STRATEGY');
        }
        
        
        $scope.setWfiNum = function(property){
        	var obj = {
                id:"WFL_NUM",
                name:property.value,
                expression:"",
        	...
        	$scope.syncFormProperties(obj,'WFL_NUM');
        }
        ...
        $scope.setApprovalAction = function(property){
            var selectedItem = $scope.selectedItem.properties;
            for(var item = 0;item<selectedItem.length;item++){
                if($scope.selectedItem.properties[item]['key'] == "oryx-formproperties"){
                    var formProperties = [];
                    if($scope.selectedItem.properties[item].value){
                        var formProperties = $scope.selectedItem.properties[item].value.formProperties || [];
                    }
                    for (var f = 0; f < formProperties.length; f++) {
                        if (formProperties[f].id == 'APPROVAL_ACTION') {
                            approvalAction = formProperties[f];
                        }
                    }
                }
            }
            var length = approvalAction.enumValues.length;
            var name = property.value?"Y":"N";
            var key= property.key;
            var id="";
            if(key == "oryx-add_sign_flag"){
                id =  "ADD_SIGN_FLAG"
            }else if(key == "oryx-delegate_flag"){
                id = "DELEGATE_FLAG";
            }
            var flag = 0;
            for(var i=0;i<length;i++){
                if(approvalAction.enumValues[i].id == id){
                    flag = 1;
                    approvalAction.enumValues[i].name = name;
                    break;
                }
            }
            if(!flag){
                approvalAction.enumValues.push({
                    id:id,
                    name:name
                })
            }
            $scope.syncFormProperties(approvalAction,'APPROVAL_ACTION');
        }

        $scope.setAutoCarbonCopy = function(property){
            var name = property.value?"Y":"N";
            var obj = {
                id:"ACT_AUTO_CARBON_COPY",
                name:name,
                expression:"",
                readable:true,
                type :"string",
            ...
            $scope.syncFormProperties(obj,'ACT_AUTO_CARBON_COPY');
        }


        $scope.setRevokeEnableFlag = function(property){
            var name = property.value?"Y":"N";
            var obj = {
                id:"ACT_REVOKE_ENABLE_FLAG",
                name:name,
                expression:"",
                readable:false,
                type :"string",
            ...
            $scope.syncFormProperties(obj,'ACT_REVOKE_ENABLE_FLAG');
        }

        $scope.syncFormProperties = function(obj,code){
        	var selectedItem = $scope.selectedItem.properties;
            for(var item = 0;item<selectedItem.length;item++){
                 if($scope.selectedItem.properties[item]['key'] == "oryx-formproperties"){
               	 var value = $scope.selectedItem.properties[item].value;
               	 if(!value || !value.formProperties || !value.formProperties.length){
               		 $scope.selectedItem.properties[item].value = {};
               		 $scope.selectedItem.properties[item].value.formProperties = [];
               		 $scope.selectedItem.properties[item].value.formProperties.push(obj);
               	 }else{
               		 var formProperties = $scope.selectedItem.properties[item].value.formProperties;
               		 var hasAssignment = false;
               		 for(var f = 0;f< formProperties.length;f++){
               			 if(formProperties[f].id == code){
               				    hasAssignment = true;
                         		$scope.selectedItem.properties[item].value.formProperties[f] = obj;
                         }
               		 }
               		 if(!hasAssignment){
               			 $scope.selectedItem.properties[item].value.formProperties.push(obj);
