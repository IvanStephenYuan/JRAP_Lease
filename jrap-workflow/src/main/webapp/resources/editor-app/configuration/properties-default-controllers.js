...
var KisBpmBooleanPropertyCtrl = ['$scope', function ($scope) {
    ...
    $scope.changeValue = function() {
        ...
        if($scope.property.key === 'oryx-add_sign_flag' || $scope.property.key === 'oryx-delegate_flag'){
            $scope.setApprovalAction($scope.property);
        }
        if($scope.property.key === 'oryx-auto_carbon_copy_flag'){
            $scope.setAutoCarbonCopy($scope.property);
        }
        if($scope.property.key === 'oryx-revoke_enable_flag'){
            $scope.setRevokeEnableFlag($scope.property);
        }
...
var KisBpmTextPropertyCtrl = [ '$scope', '$modal', function($scope, $modal) {
    ...
    var opts = {
        template:  'activiti-editor/configuration/properties/text-popup.html?version=' + Date.now(),
        scope: $scope
    };
