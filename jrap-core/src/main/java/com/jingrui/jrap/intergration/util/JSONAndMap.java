...
public class JSONAndMap {
    ...
    * */
    public static String jsonToXml(String json, String namespace) throws JrapApiException {
    ...
    * json 转 map
    * */
    private static Map jsonToMap(String inbound) throws JrapApiException{
        Map<String, Object> map = new HashMap<>();
        try {
            map = objectMapper.readValue(inbound,map.getClass());
        } catch (IOException e) {
            throw new JrapApiException(JrapApiException.ERROR_MAP_TO_XML,"转换失败");
        }
    ...
    * map转成xml
    * */
    public static String map2Xml(Map map, String namespace) throws JrapApiException {
        ...
        try {
            ...
            throw new JrapApiException(JrapApiException.ERROR_MAP_TO_XML,"格式错误");
        }
    ...
    * 把返回的xml转化成map
    * */
    public static Map xml2map(String xmlString) throws JrapApiException {
        Document doc = null;
        ...
        try {
            ...
            if (rootElement.elements().size() == 0) {
                if (rootElement.getNamespace().getText() != null && rootElement.getNamespace().getText() != "") {
                    ...
                    map.put(getName(rootElement), rootElement.getText());
                }
            } else {
            ...
            throw new JrapApiException(JrapApiException.ERROR_XML_TO_MAP,"格式错误");
        }
        return map;
    }
    ...
    private static void ele2map(Map map, Element ele) {
        ...
        if (elements.size() == 1) {
            if (elements.get(0).elements().size() == 0) {
                map.put(getName(elements.get(0)), elements.get(0).getText());
            } else {
