...
import com.jingrui.jrap.intergration.service.AuthenticationAdapter;
...
/**
 * Created by Qixiangyu on 2017/3/28.
 */
@Component
public class JrapAuthenticationServiceImpl implements IJrapAuthenticationService, BaseConstants {

    private Logger logger = LoggerFactory.getLogger(JrapAuthenticationServiceImpl.class);

    private final Integer DEFAULT_TOKEN_EXPIRE_TIME = 24 * 60 * 60;

    private final Integer DEFAULT_REFESH_TOKEN_EXPIRE_TIME = 30 * 24 * 60 * 60;

    private final String redisCatalog = JRAP_CACHE + "interface:auth:";
    ...
    private void updateRedisRecord(Map response, String atokenKey, String rtokenKey, String accessToken) {
        ...
        if (refeshObj != null) {
            redisTemplate.opsForValue().set(rtokenKey, refeshObj.toString(), DEFAULT_REFESH_TOKEN_EXPIRE_TIME,
                    TimeUnit.SECONDS);
        }
    }
